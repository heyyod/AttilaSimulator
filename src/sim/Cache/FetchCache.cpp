		...
		GPU_DEBUG(
			...
			else
			{
				...
				gpu3d::GPUStatistics::StatisticsManager::instance().LogCacheAccess(
					...
					line, way, cycle);
#if 0
				std::cout << '\n' << name << " FETCH FAILED\nADDR: " << address << ", WAY: " << way << ", SET: " << line << std::endl;
				for (u32bit row = 0; row < numLines; row++)
				{
					std::cout << row << ":\t";
					for (u32bit col = 0; col < numWays; col++)
					{
						std::cout << reserve[col][row];
						std::cout << "\t|\t";
					}
					std::cout << std::endl;
				}
...
#if KONDAMASK
				gpu3d::GPUStatistics::StatisticsManager::instance().LogCacheAccess(
					this->name, address, GPUStatistics::StatisticsManager::CACHE_FETCH_FAIL, line, way, cycle);
#endif
