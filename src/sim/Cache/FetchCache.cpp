...
#if KONDAMASK_CACHE_DECAY
	{
		// If we hit, check if it's waitForDecay.
		// If it is, ingore the hit and treat as miss.
		if (hit && isDecayed(line, way))
		{
			hit = false;
	...
	for (line = 0; (line < numLines) && (freeRequests > 0); line++)
	{
		for (way = 0; (way < numWays) && (freeRequests > 0); way++)
		{
			...
			if (valid[way][line])
			{
				...
				/*  Add new request to memory request queDue.  */
				requestQueue[freeRequest].outAddress = oldAddress;
				requestQueue[freeRequest].line = line;
				requestQueue[freeRequest].way = way;
				requestQueue[freeRequest].fill = FALSE;
...
{
	u32bit way = requestQueue[cacheRequest].way;
	u32bit line = requestQueue[cacheRequest].line;
	u32bit i;
...
u32bit FetchCache::nextVictim(u32bit line)
{
	...
	if (name[0] == 'C' || name[0] == 'Z')
	{
		...
		for (i = 0; i < maxLRU; i++)
		{
			...
			if (reserve[victim[line][i]][line] == 0 && !waitForDecay[victim[line][i]][line])
...
#if KONDAMASK
inline bool FetchCache::isDecayed(u32bit line, u32bit way)
{
	return (
		valid[way][line] &&
...
void FetchCache::decayWay(u32bit line, u32bit way)
{
	if (!KONDAMASK_CACHE_DECAY)
		return;
...
void FetchCache::decay(u64bit cycleIn, u32bit decayCycles)
{
	if (!KONDAMASK_CACHE_DECAY_PER_CYCLE)
		return;
		...
		for (u32bit w = 0; w < numWays; w++)
		{
				...
				!waitForDecay[w][l])
			{
				...
				if (cycle - accessCycles[w][l].lastHit + 1 > decayCycles)
				{
					...
					{
						gpu3d::GPUStatistics::StatisticsManager::instance().LogCacheAccess(
							this->name, address, GPUStatistics::StatisticsManager::CACHE_DECAY_RESERVED,
					...
					{
						gpu3d::GPUStatistics::StatisticsManager::instance().LogCacheAccess(
							this->name, address, GPUStatistics::StatisticsManager::CACHE_DECAY_REPLACING,
							l, w, cycle, accessCycles[w][l].insert, accessCycles[w][l].lastHit, accessCycles[w][l].lastOn);
						return;
					}
					*/
...
/*  Copy back all the valid cache lines to memory.  
	Line passed MUST be valid.
	*/
bool FetchCache::flushForDecay(u32bit line, u32bit way)
{
	/*  Get next free request queue entry.  */
	u32bit freeRequest = freeRequestList[nextFreeRequest];
	/*  Get address of the line to be replaced.  */
	u32bit oldAddress = line2address(way, line);
	...
	GPUStatistics::StatisticsManager::instance().LogCacheAccess(
		name, oldAddress, GPUStatistics::StatisticsManager::CACHE_DECAY_FLUSH_FAILED,
		line, way, cycle);
