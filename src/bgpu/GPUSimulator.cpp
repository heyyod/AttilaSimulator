...
namespace gpu3d
{
	...
	GPUSimulator::~GPUSimulator()
	{
		...
		GPUStatistics::StatisticsManager::instance().SaveOutputFiles();
	...
	void GPUSimulator::simulationLoop()
	{
		...
		for (cycle = 0, end = false, dotCount = 0; !end; cycle++)
		{
			...
			if (commProc->isSwap())
			{
				...
				// TODO: These indices are NOT necesarily correct.
				// I assume that the number of units will not change.
				...
				// 21 Total
				GPUStatistics::StatisticsManager::cache_decay_stats stats[21] = {};
				
				GPUStatistics::StatisticsManager::cache_decay_stats &in = stats[0];
				boxArray[2]->onEndOfFrame(frameCycles, &in); // Streamer
